...
const validateMean = (mean, n) => {
  ...
  if (mean.length !== n) {
    throw new Error(
      `Expected mean to have length ${n}, but had length ${mean.length}`
    );
  }
...
const validateCovAndGetSVD = (cov, n) => {
  ...
  if (cov.length !== n) {
    throw new Error(
      `Covariance matrix had ${cov.length} rows, but it should be a ${n}x${n} square matrix`
    );
  }
  ...
  cov.forEach((row, idx) => {
    ...
    if (row.length !== n) {
      throw new Error(
        `Row ${idx} of covariance matrix had length ${row.length}, but it should have length ${n}`
      );
    }
    ...
    if (!every(row, isNumber)) {
      throw new Error(
        `Row ${idx} of covariance matrix contained a non-numeric value`
      );
    }
  });
